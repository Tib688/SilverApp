  ...
  el.innerHTML = `
    ...
    <div class="chat-layout fade-in">
      <div class="chat-sidebar">
        ...
        ${testers.map(t => `
          <div class="chat-sidebar-item" id="chatSideItem-${t.code}" onclick="chatSwitchDm('${t.code}', '${esc(t.discord_username || t.code)}')">
...
function chatSwitchDm(testerCode, name) {
  chatMode = 'dm';
  chatDmTarget = testerCode;
  document.querySelectorAll('.chat-sidebar-item').forEach(e => e.classList.remove('active'));
  const item = document.getElementById(`chatSideItem-${testerCode}`);
  if (item) item.classList.add('active');
  document.getElementById('chatHeaderTitle').textContent = `✉ ${name}`;
  chatLoadMessages();
}

function chatResolveAvatarId(r) {
  if (r.sender_type === 'owner') return '1504594533521031219';
  const match = chatTesters.find(t => t.discord_username === r.sender || t.code === r.sender);
...
async function chatLoadMessages() {
  ...
  try {
    if (chatMode === 'general') {
      rows = await dbQuery("SELECT id, sender, sender_type, message, file_url, file_name, created_at FROM tester_chat ORDER BY created_at DESC LIMIT 50");
    } else {
      rows = await dbQuery("SELECT id, sender, sender_type, message, file_url, file_name, created_at FROM tester_dms WHERE tester_code = %s ORDER BY created_at DESC LIMIT 50", [chatDmTarget]);
    }
  } catch { rows = []; }

  if (!Array.isArray(rows) || rows[0]?.error) rows = [];
  rows.reverse();
  ...
  const avatarIds = [...new Set(rows.map(chatResolveAvatarId).filter(Boolean))];
  const userMap = await fetchUsersBatch(avatarIds);
  container.innerHTML = rows.map(r => {
    ...
    const avatarId = chatResolveAvatarId(r);
    ...
    const avatarHtml = avatarUrl
      ...
      : `<div class="chat-row-avatar" style="display:flex;align-items:center;justify-content:center;font-size:11px;font-weight:700;color:var(--muted)">${esc((r.sender || '?')[0].toUpperCase())}</div>`;
    return `<div class="chat-row ${isOwner ? 'chat-row-owner' : ''}">
      ${avatarHtml}
      <div class="chat-bubble ${isOwner ? 'chat-bubble-owner' : ''}">
        <div class="chat-bubble-header">
          <span class="chat-bubble-name" style="color:${isOwner ? 'var(--accent)' : 'var(--cyan)'}">${esc(r.sender)}</span>
...
async function chatSend() {
  ...
  let res;
  if (chatMode === 'general') {
    res = await dbQuery("INSERT INTO tester_chat (sender, sender_type, message, created_at) VALUES (%s, %s, %s, NOW())", ['Tib', 'owner', msg]);
  } else {
    res = await dbQuery("INSERT INTO tester_dms (tester_code, sender, sender_type, message, created_at) VALUES (%s, %s, %s, %s, NOW())", [chatDmTarget, 'Tib', 'owner', msg]);
  }
...
async function chatUploadFile(input) {
  ...
  try {
    ...
    if (chatMode === 'general') {
      await dbQuery("INSERT INTO tester_chat (sender, sender_type, message, file_url, file_name, created_at) VALUES (%s, %s, %s, %s, %s, NOW())", ['Tib', 'owner', '', data.url, data.name]);
    } else {
      await dbQuery("INSERT INTO tester_dms (tester_code, sender, sender_type, message, file_url, file_name, created_at) VALUES (%s, %s, %s, %s, %s, %s, NOW())", [chatDmTarget, 'Tib', 'owner', '', data.url, data.name]);
    }
    chatLoadMessages();
  } catch (e) {}
}
...
const SILVERAPP_CHANGELOGS = [
    ...
    { version: 'v2.3.2', date: '30/06/2026', tag: '🧪 Test Lab', color: 'var(--cyan)', items: [
      ...
      '🐛 Fix critique du Chat Testeurs : les requetes utilisaient des colonnes inexistantes en base (sender_id/receiver_id), le chat etait casse depuis le debut',
      '🖼️ Vraies photos de profil sur chaque message du chat (plus de cercle generique)',
    ...
    { version: 'v2.3.1', date: '30/06/2026', tag: '🔧 Patch', color: 'var(--cyan)', items: [
      ...
      '📋 Page Mise a jour avec changelog integre + badge violet',
      ...
      '💀 Skeleton loading chrome + horloge stylisee',
      '💬 Fix chat/DM/TestLab testeur + avatars Discord',
      '✅ Comparaison semver pour detection de mise a jour',
    ]},
